...
// Lista as subchaves (caminhos completos) de uma chave do registro.
async function regSubkeys(keyPath) {
  const r = await run(`reg query "${keyPath}"`);
  if (!r.ok) return [];
  const up = keyPath.toUpperCase() + "\\";
  return r.stdout.split(/\r?\n/).map((l) => l.trim())
    .filter((l) => l && l.toUpperCase().startsWith(up));
}

module.exports = { run, regGet, regSet, regDelete, regSubkeys };
