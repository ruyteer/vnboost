...
async function applyInstruction(id, instr, log) {
  ...
  if (instr.special === "svchostsplit") {
    const kb = Math.floor(require("os").totalmem() / 1024);
    const pth = "HKLM\\SYSTEM\\CurrentControlSet\\Control";
    original.push({ path: pth, name: "SvcHostSplitThresholdInKB", prev: await regGet(pth, "SvcHostSplitThresholdInKB") });
    instr.__svckb = kb;
  }
  let nagleSubs = [];
  ...
  if (instr.special === "svchostsplit") {
    await regSet("HKLM\\SYSTEM\\CurrentControlSet\\Control", "SvcHostSplitThresholdInKB", "REG_DWORD", String(instr.__svckb));
    say(`  > SvcHost split ajustado para ${instr.__svckb} KB (menos processos svchost).`);
  }
