...
const { run, regGet, regSet, regDelete } = require("./runner");
const backup = require("./backup");

// Curvas MarkC 1:1 (DPI/escala 100%).
const MARKC_X =
...
// Motor
// ===========================================================================
function find(id) { return TWEAKS.find((t) => t.id === id); }

async function applyTweak(id, log) {
  const t = find(id);
  if (!t) return { ok: false, msg: "Tweak desconhecido: " + id };
  const say = log || (() => {});
  say(`Aplicando "${t.name}"...`);

  if (t.reg && t.reg.length) {
    const original = [];
    for (const r of t.reg) {
  ...
  say(t.action ? `"${t.name}" executado.` : `"${t.name}" aplicado.`);
  return { ok: true };
}

async function revertTweak(id, log) {
  const t = find(id);
  if (!t) return { ok: false, msg: "Tweak desconhecido: " + id };
  const say = log || (() => {});
  if (t.action) { say(`"${t.name}" e uma acao unica (nao tem reverter).`); return { ok: true }; }
  say(`Revertendo "${t.name}"...`);
  ...
  if (t.revert) {
    for (const c of t.revert) {
      const res = await run(c);
      if (!res.ok) say(`  ! reversao: ${c.split(" ")[0]} retornou erro`);
    }
  }
  backup.clear(id);
  say(`"${t.name}" revertido.`);
  return { ok: true };
}

// ---- Jogos ----
function listGames() { return GAMES.map((g) => ({ name: g.name })); }

async function applyGame(name, log) {
  const g = GAMES.find((x) => x.name === name);
  if (!g) return { ok: false };
  const say = log || (() => {});
  say(`Priorizando "${g.name}"...`);
  const original = [];
  for (const exe of g.exes) {
  ...
  say(`"${g.name}" com prioridade Alta.`);
  return { ok: true };
}

async function revertGame(name, log) {
  const g = GAMES.find((x) => x.name === name);
  if (!g) return { ok: false };
  const say = log || (() => {});
  say(`Revertendo prioridade de "${g.name}"...`);
  ...
  if (saved && saved.reg) {
    ...
    for (const exe of g.exes) await regDelete(`${IFEO}\\${exe}\\PerfOptions`, "CpuPriorityClass");
  }
  backup.clear("game:" + name);
  say(`"${g.name}" revertido.`);
  return { ok: true };
}
...
module.exports = { list, applyTweak, revertTweak, setEslPath, listGames, applyGame, revertGame };
